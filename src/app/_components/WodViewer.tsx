...
export type Wod = {
  ...
  tags?: Array<
    "Chipper" | "Couplet" | "Triplet" | "EMOM" | "AMRAP" | "For Time" | "Ladder"
  >;
};
...
export const getPerformanceLevelTooltip = (wod: Wod): string => {
  ...
  levelOrder.forEach((levelName) => {
    ...
    if (type === "time") {
      ...
      const min =
        levelData.min !== null ? formatSecondsToMMSS(levelData.min) : "0:00"; // Usually 0 for time min
      const max =
...
export const sortWods = (
  ...
  return [...wodsToSort].sort((a, b) => {
    ...
    if (sortBy === "wodName") {
      ...
      // Combine level and latestLevel logic slightly
...
export default function WodViewer({
  ...
  const doneWodsCount = wods.filter(
    (wod) => wod.results.some((r) => r.date && hasScore(r)), // Check if *any* result is valid
  ).length;
